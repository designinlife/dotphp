...
PHP_METHOD(DotPHP_DB_DbPdo, __construct) {
	...
	ZEPHIR_CALL_PARENT(NULL, dotphp_db_dbpdo_ce, this_ptr, "__construct", &_0, 4, bootstrap);
	...
	if (Z_TYPE_P(sql) == IS_ARRAY) {
		...
		if (EG(exception)) {
			...
			if (zephir_instance_of_ev(e, php_pdo_get_exception() TSRMLS_CC)) {
				...
				zephir_check_call_status();
				ZEPHIR_CALL_METHOD(NULL, _7, "__construct", NULL, 17, _6, _8, v);
				zephir_check_call_status();
		...
		if (EG(exception)) {
			...
			if (zephir_instance_of_ev(e, php_pdo_get_exception() TSRMLS_CC)) {
				...
				zephir_check_call_status();
				ZEPHIR_CALL_METHOD(NULL, _7, "__construct", NULL, 17, _8, _9, sql);
				zephir_check_call_status();
	...
	if (ZEPHIR_IS_FALSE(_1)) {
		...
		if (EG(exception)) {
			...
			if (zephir_is_instance_of(e, SL("DotPHP\\DB\\PDOException") TSRMLS_CC)) {
				...
				ZEPHIR_CALL_METHOD(NULL, _18, "__construct", NULL, 17, _17);
				zephir_check_call_status();
